...
public class TournamentService {
    ...
    public Tournament createTournament(CreateTournamentRequest request, Long createdById) {
        ...
        var creator = playerRepository.findById(createdById)
    ...
    public Tournament getTournamentWithRegistrations(Long tournamentId) {
        return tournamentRepository.findByIdWithRegistrations(tournamentId)
                .orElseThrow(() -> new EntityNotFoundException("Tournament not found with id: " + tournamentId));
    }

    public Tournament getTournamentWithMatches(Long tournamentId) {
        return tournamentRepository.findByIdWithMatches(tournamentId)
                .orElseThrow(() -> new EntityNotFoundException("Tournament not found with id: " + tournamentId));
    }
    ...
    public Tournament startTournament(Long tournamentId) {
        ...
        if (!tournament.canStart()) {
            throw new IllegalStateException("Tournament cannot be started. Check registration status and participant count.");
        }
    ...
    public TournamentRegistration registerForTournament(Long tournamentId, TournamentRegistrationRequest request) {
        ...
        if (request.isTeamRegistration()) {
            var partner = playerRepository.findById(request.partnerId())
                    .orElseThrow(() -> new EntityNotFoundException("Partner not found with id: " + request.partnerId()));
    ...
    public void withdrawFromTournament(Long tournamentId, Long playerId) {
        ...
        var registration = registrationRepository.findByTournamentIdAndPlayerId(tournamentId, playerId)
    ...
    private void generateBracket(Tournament tournament) {
        ...
        var activeRegistrations = registrationRepository
                .findByTournamentIdAndStatusOrderBySeedAscRegistrationDateAsc(
    ...
    public TournamentMatch recordWalkover(Long matchId, WalkoverRequest request) {
        ...
        var match = getMatchById(matchId);
        var winner = registrationRepository.findById(request.winnerRegistrationId())
                .orElseThrow(() -> new EntityNotFoundException("Registration not found with id: " + request.winnerRegistrationId()));
        ...
        // Advance winner to next round
        TournamentAlgorithm algorithm = getTournamentAlgorithm(match.getTournament().getTournamentType());
